...
export default function TaskListItem({ text, checked }) {
    ...
    return (
        <Box sx={{ mb: '18px', bgcolor: 'background.paper' }}>
            {changingTheNameOfTask ? (<Box
                onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                        dispatch(changeTheNameOfTask({ id: tasks[indexOfTheTask].id, newName: valueOfInput }));
                        request(`http://localhost:3001/tasks/${idOfTheTask}`, 'PUT', JSON.stringify({
                ...
                >
                    <FormControlLabel
                        ...
                        label={<Typography sx={{ color: "text.primary" }}>
                            {text}
                        </Typography>}
                    />

                    <IconButton onClick={focusedTask ? closeAdditionalContainer : openAdditionalContainer}>
                        {focusedTask ? < KeyboardArrowDownIcon
                            sx={{ fontSize: 25, color: "icons.secondary" }} /> : <KeyboardArrowRightOutlinedIcon
                ...
                <CSSTransition
                    ...
                    <Box
                        ...
                        onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                                dispatch(setTheDateOfPerfomingTheTask({ name: text, date: dayjs(date).format('DD.MM.YYYY') }))
                                request(`http://localhost:3001/tasks/${idOfTheTask}`, 'PUT', JSON.stringify({
                                    ...tasks[indexOfTheTask],
                    ...
                    >
                        ...
                        {showCalendar ? (<LocalizationProvider dateAdapter={AdapterDayjs}>
                            <DateField
                                autoFocus={true}
                                onChange={(newValue) => {
                                    setDate(newValue)
                                ...
                                label="Set date"
                            />
                        </LocalizationProvider>) : (<IconButton
                        ...
                        <IconButton onClick={() => {
                            ...
                            {indexOfTheTask !== null && tasks[indexOfTheTask] && tasks[indexOfTheTask].important ?
                                <StarIcon sx={{
                                    fontSize: 25,
                                    color: 'icons.primary'
                                }} /> :
